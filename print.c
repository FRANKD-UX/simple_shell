...
{
	return (write(STDOUT_FILENO, string, string_l(string)));
	/*Use the 'write' system call to print the string to standard output*/
}
/**
...
{
	return (write(STDERR_FILENO, string, string_l(string)));
	/*Use the 'write' system call to print the string to standard error*/
}
/**
* Prints error messages to standard output based on the provided error code.
* This function generates and prints an error message formatted with
* information about the error code, the program name, the command name,
* and additional details.
* The message is formatted differently depending on the specific error code.
* @param errorcode The error code that determines the format and content
* of the error message.
* @param data A pointer to a 'program_data' structure containing relevant
...
int printErr(int errorcode, program_data *data)
{
	...
	int_to_string((long) data->ex_count, n_as_string, 10);
	/* Convert the execution count to a string*/

	if (errorcode == 2 || errorcode == 3)
	/*Print error message based on the provided error code*/
	{
		...
		print2(data->tokens[1]);
		/*Additional information (e.g., directory name)*/
		print2("\n");
	}
	else if (errorcode == 127)/*Print error message for error code 127*/
	{
		print2(data->program_name); /* Program name*/
		print2(": ");
		print2(n_as_string); /* Execution count*/
	...
	else if (errorcode == 126)/* Print error message for error code 126*/
	{
		print2(data->program_name); /* Program name*/
		print2(": ");
		print2(n_as_string); /* Execution count*/
