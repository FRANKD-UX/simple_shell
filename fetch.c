...
int fetch(program_data *data)
{
	...
	static char *a_com[10] = {NULL};
		...
		if (!a_com[0] || (a_op[0] == '&' && errno != 0) ||  /* use of logical operators */
		...
		(a_op[0] == '|' && errno == 0))
	{
		...
		br = read(data->file_descr, &buff, BUFFER_SIZE - 1); /* file desc is used to read into to buff */
		...
		x = 0;
		do {
			a_com[x] = string_d(_strtok(x ? NULL : buff, "\n;")); /*use of new line command*/
			x = check(a_com, x, a_op);
		} while (a_com[x++]);
	}

	data->input_line = a_com[0]; /*gets next command and removes it for the array*/
	for (x = 0; a_com[x]; x++)
	{
		a_com[x] = a_com[x + 1];
		a_op[x] = a_op[x + 1];
...
int check(char *a_com[], int x, char a_op[])
{
	...
	for (y = 0; a_com[x] != NULL  && a_com[x][y]; y++) /* checks for ‘&’ in command line*/
	{
